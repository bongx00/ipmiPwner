...
def arguments():
    ...
    req.add_argument(
        "-w",
        "--wordlist",
        help="The wordlist of passwords to crack the hash",
        dest="wordlist",
        metavar="/path/to/password_wordlist",
        required=True,
    )
    req.add_argument(
        "-o",
        "--output",
        help="File where the cracking result will be saved",
        dest="output",
    ...
    parser.add_argument(
        "-l",
        "--line",
        help="Notify when the specified number of lines has been read, by default: 100,000",
        dest="top",
        metavar="number",
        default=1000000,
        type=int,
...
def checkArguments(file, output, wordlist):
    ...
    if not os.path.isfile(wordlist):
        print(f"[!] The wordlist: {file} does not exist")
        sys.exit()
...
def readByChunks(file, wordlist, chunk_size=1024**2):
    ...
    while True:
        ...
        print(
            f"[*] Reading Bytes: {str(len(content))}/{str(full_size)}" + " " * 50,
            end="\r",
        )
...
def main():
    args = arguments()
    file, output, wordlist, top, init_top = (
        args.file,
        args.output,
        args.wordlist,
        args.top,
